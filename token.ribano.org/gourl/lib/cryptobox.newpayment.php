...
function cryptobox_new_payment($paymentID = 0, $payment_details = array(), $box_status = "")
{
	...
	if (isset($payment_details['status']) && @$payment_details['status'] == 'payment_received' && $box_status == "cryptobox_newrecord") {
		...
		if (!$recordExists) {
			...
			run_sql("INSERT INTO `dbt_deposit` VALUES(NULL,'".$payment_details["user"]."',".(floatval($payment_details["amount"])-floatval($fees)).",'bitcoin',".floatval($fees).",'".$paymentID."','".gmdate("Y-m-d H:i:s")."',NULL,".intval(1).",'".@$_SERVER['REMOTE_ADDR']."','".$payment_details["coinname"]."')");
			...
			if (!$UserRecordExists) {
				run_sql("INSERT INTO `dbt_balance` VALUES(NULL,'".$payment_details["user"]."',".(floatval($payment_details["amount"])-floatval($fees)).",'".gmdate("Y-m-d H:i:s")."')");

				$UserNewRecordExists = run_sql("SELECT * FROM `dbt_balance` WHERE user_id = '".$payment_details["user"]."'");

				if ($UserNewRecordExists) {
					run_sql("INSERT INTO `dbt_balance_log` VALUES(NULL,'".@$UserNewRecordExists->id."','".$payment_details["user"]."','DEPOSIT', ".(floatval($payment_details["amount"])-floatval($fees)).",'".floatval($fees)."', '".@$_SERVER['REMOTE_ADDR']."', '".gmdate("Y-m-d H:i:s")."')");
				}
				...
				run_sql("INSERT INTO `dbt_balance_log` VALUES(NULL,'".@$UserRecordExists->id."','".$payment_details["user"]."','DEPOSIT', ".(floatval($payment_details["amount"])-floatval($fees)).",'".floatval($fees)."', '".@$_SERVER['REMOTE_ADDR']."', '".gmdate("Y-m-d H:i:s")."')");
			}
		}
