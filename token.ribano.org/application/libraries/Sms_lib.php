...
class Sms_lib
{
    ...
    public function send($config = array())
    {
        ...
        if ($sms->gatewayname == 'budgetsms') {
            ...
            if (curl_errno($curl)) {
                ...
                $checkerror = $res->requestError['serviceException']['messageId'];

                if (strpos($checkerror, "UNAUTHORIZED") === false) {
                    return json_encode(array(
                        'status'      => true,
                        'message'     => "success: " . $response
                    ));
    ...
    public function sms_rank_send($config = array())
    {
        ...
        if ($sms->gatewayname == "budgetsms") {
            ...
            if (is_array($config['template_config']) && sizeof($config['template_config']) > 0) {
                $message = $this->bulk_template($config['template'], $config['template_config']);
            }
