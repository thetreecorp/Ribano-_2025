...
class FAQ extends CI_Controller {
	...
	{  
        ...
        ******************************/
        ...
        $config["total_rows"] = $this->db->select('*')->from('web_article')->where('data_key','faq_data')->get()->num_rows();
        $config["per_page"] = 25;
        ...
        $data['article'] = $this->article_model->getmultipleArticle('faq_data');
        $data["links"] = $this->pagination->create_links();
        /******************************
        * Pagination ends
        ******************************/

		$data['content'] = $this->load->view("backend/article/faq_list", $data, true);
	...
	public function form($article_id = null)
	{
		...
		if ($this->form_validation->run()) 
		{
			...
			if(!empty($article_id)) {
				...
				$data['article_details']  = $this->article_model->allDetailsData($article_id,"faq_data");
