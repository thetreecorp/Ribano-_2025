...
class Social_app extends CI_Controller {
	...
	{ 
		...
		$this->form_validation->set_rules('app_id', display('app_id'),'required|max_length[1000]|xss_clean');
		$this->form_validation->set_rules('app_secret', display('app_secret'),'required|max_length[1000]|xss_clean');
		if ($id==2) {
			$this->form_validation->set_rules('api_key', display('api_key'),'required|max_length[1000]|xss_clean');
		}
