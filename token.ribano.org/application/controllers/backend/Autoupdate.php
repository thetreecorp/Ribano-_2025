...
//Get Update file
define('MAX_VERSION','5.9');
//Get Update file
define('UPDATE_URL','http://update.bdtask.com/tradebox/autoupdate');
// Get latest version info
define('UPDATE_INFO_URL','http://update.bdtask.com/tradebox/autoupdate/update_info');
// CRM temporary path
...
class Autoupdate extends CI_Controller
{
    ...
    public function update()
    {
        ...
        $this->form_validation->set_rules('purchase_key', display('message'),'required|max_length[100]|trim|xss_clean');
        ...
        if ($this->form_validation->run()) {
            ...
            if ($return_data['purchase_key'] == 'invalid' && $response_code==200) {
                $this->session->set_flashdata('exception', display('purchase_key_invalid'));

            }elseif($return_data['purchase_key'] == 'valid' && $response_code==200){
                $this->session->set_flashdata('message', display('software_updated_successfully'));
            }

        }else{

            $this->session->set_flashdata('exception', display('purchase_key_in_required'));            
        }
