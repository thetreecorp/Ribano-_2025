                ...
                <table class="datatable2 table table-bordered table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><?php echo html_escape($coininfo->pair_with) . " " . html_escape($value->amount); ?>
                            </td>
                            <td><?php echo html_escape($value->method); ?></td>
                            <td><?php echo html_escape($coininfo->pair_with) . " " . html_escape($value->fees_amount); ?>
                            </td>
                            <td>
                                <?php
                                    if (is_string(html_escape($value->comment)) && is_array(json_decode(html_escape($value->comment), true)) && (json_last_error() == JSON_ERROR_NONE) ? true : false) {
                                        ...
                                        echo html_escape($value->comment);
                                    }
                                    ?>
                            </td>
                            <td>
                            ...
                            <td width="150px">
                                ...
                                <?php if (!empty($userrole)) {
                                            if ($userrole->create_permission == 1) { ?>
                                <a href="<?php echo base_url() ?>backend/finance/deposit/confirm_deposit?id=<?php echo $value->id; ?>&user_id=<?php echo $value->user_id; ?>&set_status=1"
                                    class="btn btn-success btn-sm"><?php echo display('confirm') ?></a>
                                <a href="<?php echo base_url() ?>backend/finance/deposit/cancel_deposit?id=<?php echo $value->id; ?>&user_id=<?php echo $value->user_id; ?>&set_status=2?>"
