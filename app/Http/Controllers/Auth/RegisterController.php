...
//    protected $redirectTo = RouteServiceProvider::HOME;
    ...
    protected function create(array $data)
    {
       ...
       // dd($data['role']);
        ...
        $checkRole = Role::where('name', Str::slug($data['role']))->count();
        if(!$checkRole)
            Role::create(['name' => Str::slug($data['role'])]);
    ...
    public function register(Request $request)
    {
        ...
        if (request()->has('redirect_uri') && request()->input('redirect_uri')) {
            ...
            Cache::put('redirect_uri', $redirectUri, now()->addHours(24));
        } 
        ...
        return redirect()->route('newActiveAccount');
