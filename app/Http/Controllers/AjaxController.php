...
class AjaxController extends Controller
{
    ...
    public function createWithdrawRequest(Request $request)
    {
        try {
            ...
            $user_id = auth()->user()->id;
            $tokenAvaiable = getWithdrawableTokens($user_id);
            $token_symbol = $request->choose_token;
            
            if(key_exists($token_symbol, $tokenAvaiable )) {
                
                if($tokenAvaiable[$token_symbol] < $request->number_token) {
                    $data['message'] = trans('Withdrawable token amount exceeds unlock threshold');
                    $data['code'] = 400;
                    return response()->json($data, 200); 
                }
                else {
                     $findToken = ManagePlan::where('token_symbol', $token_symbol)->first();
            
                    if($findToken) {
                        $status =  WithdrawRequest::create([
                            'user_id' => $user_id, 
                            'status' => 'pending',
                        ...
                        $data['message'] = trans('Withdraw request created');
                        $data['code'] = 200;
                        return response()->json($data, 200);
                    }
                    else {
                        $data['message'] = trans('Token not found');
                        $data['code'] = 400;
                        return response()->json($data, 200); 
                    }
                }
                ...
                $data['message'] = trans('Token not withdraw');
                $data['code'] = 400;
                return response()->json($data, 200); 
            }
            // check number withdraw
    ...
    // load video
    public function ajaxLoadSpeed(Request $request)
    { 
        ...
        if($request->type == 'video')
            $returnHTML = view('partials.load-video', compact('findProject', 'imagesPerPage', 'totalPages', 'totalItem', 'videosPerPage', 'totalVideosPages', 'totalVideoItem'))->render();
        else
            $returnHTML = view('partials.load-gallery', compact('findProject', 'imagesPerPage', 'totalPages', 'totalItem', 'videosPerPage', 'totalVideosPages', 'totalVideoItem'))->render();
            
        return response()->json(array('success' => true, 'html' => $returnHTML, 'code' => 200));
    ...
    public function sendCodeToPhone(Request $request) {
      ...
      // dd(config('constants.options.check_mobile'));
        try {
           ...
           // dd($error);
            ...
            return response()->json([
                'code' => 409,
                'message' => trans('error_auth_checkmobil'),
            ]);
        }
    }
        ...
        if($request->type == 'email') {
            ...
            if($user) {
                if(!$user->checkmobi_id) {
                    ...
                    $request->merge(['id' => $user->checkmobi_id, 'pin' => $request->otp_code]);
