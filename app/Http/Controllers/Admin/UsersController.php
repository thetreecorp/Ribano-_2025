...
class UsersController extends Controller
{
    ...
    public function getUsers($type)
    {
        ...
        $users = User::whereHas("roles", function($q) use($role) { $q->where("name", $role); })->with('roles')->paginate(config('basic.paginate'));
        // dd($users);
        return view('admin.users.list', compact('users'));
    }
    
    public function pendingUsers($type)
    {
        if($type == 'investors') 
            $role = 'investor';
        else 
        ...
        $users = User::whereHas("roles", function($q) use($role) { $q->where("name", $role); })->where('status',0)->paginate(config('basic.paginate'));
        return view('admin.users.list', compact('users'));
    }
    ...
    public function importUsers($type)
    {
        ...
        $users = User::whereHas("roles", function($q) use($role) { $q->where("name", $role); })->where('import', 1)->paginate(config('basic.paginate'));
        return view('admin.users.list', compact('users'));
    }
    
    public function kycUsers($type)
    {
        if($type == 'investors') 
            $role = 'investor';
        else 
        ...
        $users = User::whereHas("roles", function($q) use($role) { $q->where("name", $role); })->orderBy('id', 'DESC')->paginate(config('basic.paginate'))->pluck('id')->toArray();
        
        $title = "Pending KYC";
        $logs = KYC::orderBy('id', 'DESC')->where('status',0)->with(['user','admin'])
    ...
    public function userEdit($id)
    {
        ...
        return view('admin.users.edit-user', $data, compact('user','languages', 'roles'));
    }
