...
class ContentController extends Controller
{
    ...
    public function create($content)
    {
        ...
        //dd($content);
        return view('admin.content.create', compact('languages', 'content', 'categories'));
    }
    ...
    public function store(Request $request, $content, $language)
    {
        ...
        if ($language != 0) {
          ...
          //  dd($purifiedData['category_id']);
            $contentDetails->category_id = $purifiedData['category_id'] ?? NULL;
            $contentDetails->save();
        }
    ...
    public function show(Content $content, $name = null )
    {
        ...
        return view('admin.content.show', compact('content', 'languages', 'contentDetails', 'contentMedia', 'categories'));
    }
    ...
    public function update(Request $request, Content $content, $language)
    {
        ...
        //$purifiedData = Purify::clean($request->except('image', 'thumbnail', '_token', '_method'));
        $purifiedData = Purify::clean($request->except('thumbnail', '_token', '_method'));
        ...
        if ($request->hasAny(array_keys(config('contents.content_media')))) {
            ...
            foreach (config('contents.content_media') as $key => $media) {
                ...
                if ($request->hasFile($key)) {
                    ...
                    $contentMediaDescription[$key] = $this->uploadImage($purifiedData[$key][$language], config('location.content.path'), $size, $old_data, $thumb, $old_data);
                } elseif ($request->has($key)) {
                    $contentMediaDescription[$key] = linkToEmbed($purifiedData[$key][$language]);
            ...
            //dd($contentMediaDescription);


            $contentMedia->content_id = $content->id;
            $contentMedia->description = $contentMediaDescription ? $contentMediaDescription : null;
            $contentMedia->save();
        }
