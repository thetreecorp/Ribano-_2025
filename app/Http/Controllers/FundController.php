...
class FundController extends Controller
{
    ...
    public function myInvestors()
    {
       ...
       // $paymoneys = PayMoney::where(['user_id' => $user_id])->pluck('project_id')->toArray();
        ...
        // custom 
        // get project by user
        $projectIds = Project::where('user_id', $this->user->id)
        ->distinct()
        ->pluck('id')
        ->toArray();
        ...
        if(count($projectIds)) {
            $userIds = Paymoney::whereIn('project_id', $projectIds)
            ->distinct()
            ->pluck('user_id')
            ->toArray();
        }
        ...
        // update shortlist
        $userlists = User::whereIn('id', $userIds)->paginate(12);
        
        $total_userlists = $userlists->total();

        $total = $shortlists->total();

        $total_interesteds = $interesteds->total();
        return view('fundraising.my_investors',compact('shortlists', 'interesteds', 'total', 'total_interesteds', 'userlists', 'total_userlists'));    
    }
