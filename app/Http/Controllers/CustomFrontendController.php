...
class CustomFrontendController extends Controller
{
    ...
    public function newActiveAccount()
    {  
        return view('active-account.active-account');
    ...
    public function entrepreneursFaqsPage()
    {  
        $faqs = Faq::where('status', 1)->where('category_id', '2')->get();
        return view('faq-page.index', compact('faqs'));
        
    }
    public function investorsFaqsPage()
    {  
        $faqs = Faq::where('status', 1)->where('category_id', '1')->get();
        return view('faq-page.index', compact('faqs'));
    ...
    {  
        ...
        $templateSection = ['privacy-page'];
        $data['templates'] = $templates = Template::templateMedia()->whereIn('section_name', $templateSection)->get()->groupBy('section_name');
        
        $privacy = @$templates['privacy-page'][0]->description;
        
        return view('pages.privacy', compact('privacy'));
        
    }
    
    public function termPolicies()
    {  
        
        $templateSection = ['term-policies-page'];
        $data['templates'] = $templates = Template::templateMedia()->whereIn('section_name', $templateSection)->get()->groupBy('section_name');
        
        $privacy = @$templates['term-policies-page'][0]->description;
        
        return view('pages.term-policies', compact('privacy'));
        
    }
    
    public function investMethod()
    {  
        
        $templateSection = ['invest-methods-page'];
        $data['templates'] = $templates = Template::templateMedia()->whereIn('section_name', $templateSection)->get()->groupBy('section_name');
        
        $config = @$templates['invest-methods-page'][0]->description;
        return view('pages.investor-method', compact('config'));
        
    }
    
    public function investorTerms()
    {  
        
        $templateSection = ['investor-terms-page'];
        $data['templates'] = $templates = Template::templateMedia()->whereIn('section_name', $templateSection)->get()->groupBy('section_name');
        
        $config = @$templates['investor-terms-page'][0]->description;
        return view('pages.investor-terms', compact('config'));
        
    }
    public function investmentStrategies()
    {  
        
        $templateSection = ['investment-strategies'];
        $data['templates'] = $templates = Template::templateMedia()->whereIn('section_name', $templateSection)->get()->groupBy('section_name');
        
        $config = @$templates['investment-strategies'][0]->description;


        return view('pages.investment-strategies', compact('config'));
