...
class ChatController extends Controller
{
    ...
    {
        $bidder = User::where('username', $request->user_name)->first();
        //dd($bidder);
    ...
    public function chat_index(Request $request)
    {
       ...
       // dd($bidder);
        ...
        $userId = Auth::user()->id;
        $chat_threads = ChatThread::where(function ($query) use ($userId) {
                $query->where('sender_user_id', $userId)
                      ->orWhere('receiver_user_id', $userId);
            })
            ->whereColumn('sender_user_id', '!=', 'receiver_user_id')
            ->get();
        
        return view('chat.messages', compact('chat_threads'));
    }
