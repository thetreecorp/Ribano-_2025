...
class HomeController extends Controller
{
    ...
    public function profile(Request $request)
    {
        ...
        $data['project_stage'] = ProjectStage::all();
        $data['industry_category'] = IndustryCategory::all();
    ...
    public function investorInfo(Request $request) {
        try {
          ...
          //  dd($req);
            ...
            $user->network = $req['network'];
            $user->project_stage = isset($req['project_stage']) ?  implode(',', $req['project_stage']) : NULL;
            $user->industry_category = isset($req['industry_category']) ? implode(',', $req['industry_category']) : NULL;
            $user->location =  isset($req['location']) ? implode(',', $req['location']) : NULL;
            $user->countries =  isset($req['countries']) ? implode(',', $req['countries']) : NULL;
            $user->about = $req['about'];
            $user->my_area = $req['my_area'];
    ...
    {
        $investments = $this->user->payMoneys()->groupBy('project_id')->selectRaw('*, sum(paymoneys.total) as totalToken')->orderBy('paymoneys.id','DESC')->paginate(config('basic.paginate'));
    ...
    public function withdraw(Request $request)
    {
        ...
        $userTokens->getCollection()->transform(function ($log)  use ($withdrawnTokens) {
            ...
            $withdrawnToken = $withdrawnTokens[$symbol] ?? 0;
            $log->withdrawableTokens = max(0, $withdrawableTokens - $withdrawnToken);
