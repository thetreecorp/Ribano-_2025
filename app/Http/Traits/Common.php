...
trait Common
{
    ...
    public function searchProjectFunc($args)
    {
        ...
        if (array_key_exists('min-price', $args) && $args['min-price'] && array_key_exists('max-price', $args) && $args['max-price']) {
            ...
            $projects  = $projects->whereBetween(DB::raw('CAST(replace(replace(replace(minimum_equity_investment, ".", ""), ",", ""), " ", "") AS UNSIGNED)'), [(int)$minPrice, (int)$maxPrice]);
        }
    ...
    function getTokenSymbol($project_id) {
        $plan = ManagePlan::where('project_id', $project_id)->get('token_symbol')->first();
        if($plan)
            return $plan->token_symbol;
        return 'N/A';
