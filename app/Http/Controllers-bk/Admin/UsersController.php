...
class UsersController extends Controller
{
    ...
    public function getUsers($type)
    {
        ...
        $users = User::whereHas("roles", function($q) use($role) { $q->where("name", $role); })->paginate(config('basic.paginate'));
        return view('admin.users.list', compact('users'));
    }
    ...
    public function pendingUsers($type)
    {
        ...
        $users = User::whereHas("roles", function($q) use($role) { $q->where("name", $role); })->where('status', 2)->paginate(config('basic.paginate'));
        return view('admin.users.list', compact('users'));
    }
