...
class ProjectController extends Controller
{
    ...
    public function adminProject(Request $request)
    {
        if($request->type == 'pending')
            $projects = Project::where('status', 2)->orderBy('id', 'DESC')->paginate(config('basic.paginate'));
        else
            $projects = Project::orderBy('id', 'DESC')->paginate(config('basic.paginate'));
        return view('project.admin-project', compact('projects'));
