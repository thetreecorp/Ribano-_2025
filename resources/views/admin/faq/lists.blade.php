        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header modal-colored-header bg-primary">
                    <h5 class="modal-title">@lang('Add Faq')</h5>
                ...
                <form action="" id="fsm-category" method="post">
                    ...
                    <div class="modal-body">
                        <div class=" col-md-12">
                            <div class="form-group">
                                <label>{{trans("Faq title")}}</label>
                                <input id="faq_title" required type="text" name="name" value="" placeholder="Faq title" class="form-control">
                            </div>
                        </div>
                        <div class=" col-md-12">
                            <div class="form-group">
                                <label>{{trans("Faq description")}}</label>
                                <textarea class="form-control summernote" id="faq_desc" name="description" rows="5"></textarea>
                            </div>
                        </div>
                        <div class=" col-md-12">
                            <div class="form-group">
                                <label>{{trans("Category")}}</label>
                                <select  class="form-control" name="faq-category" id="faq_cat">
                    ...
                    <div class="modal-footer">
                        <a href="javascript:void(0)" class="btn btn-light" data-dismiss="modal"><span>@lang('Cancel')</span></a>
                        <button id="submit-category-btn" type="submit" href="" class="btn btn-primary active-yes"><span>@lang('Submit')</span></button>
                    </div>
                </form>
            </div>
        </div>
                ...
                <form action="" id="fsm-edit-category" method="post">
                    ...
                    <div class="modal-body">
                        <div class=" col-md-12">
                            <div class="form-group">
                                <label>{{trans("Faq title")}}</label>
                                <input id="edit_faq_title" required type="text" name="name" value="" placeholder="Faq title" class="form-control">
                            </div>
                        </div>
                        <div class=" col-md-12">
                            <div class="form-group">
                                <label>{{trans("Faq description")}}</label>
                                <textarea class="form-control summernote" id="edit_faq_desc" name="description" rows="5"></textarea>
                            </div>
                        </div>
                        <div class=" col-md-12">
                            <div class="form-group">
                                <label>{{trans("Category")}}</label>
                                <select  class="form-control" name="faq-category" id="edit_faq_cat">
                                    @foreach ($categories as $cat)
                                        <option value="{{$cat->id}}">
                                            {{$cat->name}}
                                        </option>
                    ...
                    <div class="modal-footer">
                        <a href="javascript:void(0)" class="btn btn-light" data-dismiss="modal"><span>@lang('Cancel')</span></a>
                        <button id="submit-edit-category-btn" type="submit" href="" class="btn btn-primary active-yes"><span>@lang('Submit')</span></button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $( "#fsm-category" ).submit(function(e) {
                ...
                $.ajax({
                    type: 'post',
                    url: "{{ route('admin.createFaqContent') }}",
                    data: {
                        'name' : $('#faq_title').val(),
                        'description' : $('#faq_desc').val(),
                        'category_id' : $('#faq_cat').val(),
                ...
                $.ajax({
                    ...
                    success:function(data){
                        if(data.success == 1) {
                            ...
                            $('#edit_faq_title').val(data.data.name);
                            $('#edit_faq_cat').val(data.data.category_id);
                            $('#edit_faq_desc').summernote('code', data.data.description);
                        }
                        else {
            ...
            $( "#fsm-edit-category" ).submit(function(e) {
                ...
                $.ajax({
                    type: 'post',
                    url: "{{ route('admin.saveEditFaqContent') }}",
                    data: {
                        'name' : $('#edit_faq_title').val(),
                        'description' : $('#edit_faq_desc').val(),
                        'category_id' : $('#edit_faq_cat').val(),
                        '_token' : $('meta[name="csrf-token"]').attr('content'),
                        'type': 'edit',
                        'id' : id,
            ...
            Swal.fire({
              ...
              if (result.isConfirmed) {
                ...
                $.ajax({
                    type: 'post',
                    url: "{{ route('admin.deleteFaqContent') }}",
                    data: {
                        'id' : id,
                        '_token' : $('meta[name="csrf-token"]').attr('content'),
